...
for await (const file of walk("lib")) {
	js += `if (!(await app.vault.adapter.exists(join(dirPath, "${file.path}")))) {\n`;

	if (file.isDirectory) {
		js += `await app.vault.adapter.mkdir(join(dirPath, "${file.path}"));\n`;
	}

	if (file.isFile) {
		const base64 = encodeBase64(await Deno.readFile(file.path));
		js += `await app.vault.adapter.writeBinary(join(dirPath, "${file.path}"), base64ToArrayBuffer("${base64}"));\n`;
	}
